...
class VaDE(nn.Module):
    ...
    def pre_train(self, pre_epoch=10):
        if not os.path.exists('./c5_vade2.pkl'):
            Loss = nn.MSELoss()
            opti = Adam(itertools.chain(self.encoder.parameters(), self.decoder.parameters()), lr=1e-3)

            print('Pretraining......')
            epoch_bar = tqdm(range(pre_epoch))
            x_all_loader, X_train_all, _, y_train_all, _ = get_pworkload(self.X, self.Y, std='minmax', series_len=self.args.series_len, step=self.args.step, batch_size=self.args.batch_size)
            for i in epoch_bar:
                L = 0
                #for x in x_all:
                if self.args.cuda:
            ...
            aic_l = []
            # bic_l = []
            # for n_c in tqdm(range(1, 600, 50)):
            ...
            # plt.ylabel('bic')
            # plt.xticks(range(len(bic_l)), labels=range(1, 600, 50))
            ...
            torch.save(self.state_dict(), './c5_vade2.pkl')
            # pickle.dump([X_train_all, y_train_all, pre_res], open(r'train_data_cluster.pkl', 'wb'))
            # pickle.dump(pre_res, open(r'ns_cluster_res_n{}_s{}_s{}'.format(best_n, self.args.series_len, self.args.step), 'wb'))
            return best_n
        else:
            self.load_state_dict(torch.load('c5_vade2.pkl'))
    ...
    def predict(self, x, type):
        ...
        pickle.dump([X_raw_all, y_raw_all, np.argmax(yita, axis=1)],
                    open(r'test_data_cluster_{}.pkl'.format(type), 'wb'))

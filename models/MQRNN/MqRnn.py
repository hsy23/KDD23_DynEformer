...
def train(X, y, args, quantiles):
    ...
    for epoch in tqdm(range(args.num_epoches)):
        ...
        for step in range(int(len(X_train_all)/args.batch_size)):
            ...
            X_train_batch, y_train_batch, Xf, yf = X_train_all[be:end, :, :], Y_train_all[be:end, :], \
                                     Xf_all[be:end, :, :], Yf_all[be:end, :]
            X_train_tensor = torch.from_numpy(X_train_batch).float().to(device)
            y_train_tensor = torch.from_numpy(y_train_batch).float().to(device)
            ...
            if (epoch % 5 == 0 and step == 0):
                ...
                print("Quantile loss:{}".format(loss.item()))
    ...
    model = model.cpu()
    pickle.dump(model, open("MqRNN_{}.pkl".format(time.strftime("%m%d%H%M", time.localtime())), 'wb'))
    mape_list = []
    num_ts = Xte.shape[0]
    ...
    print('The Mean Squared Error of forecasts is {}'.format(((ypred[:, :, 1].reshape(-1) - yf_test.reshape(-1)) ** 2).mean()))
    print('The Root Mean Squared Error of forecasts is {}'.format(np.sqrt(((ypred[:, :, 1].reshape(-1) - yf_test.reshape(-1)) ** 2).mean())))

    # if args.show_plot:
    #     show_idx = 0
    #     plt.figure(1, figsize=(20, 5))
    #     plt.plot([k + seq_len + num_obs_to_train - seq_len for k in range(seq_len)], ypred[show_idx, :, 1], "r-")
...
if __name__ == "__main__":
    ...
    if args.run_test:
        # data_path = util.get_data_path("all_mac_825_831_pro.pkl")
        ...
        #
        # for i in range(0, len(tmp3), num_periods):
        #     tmp4 = tmp3.iloc[i:i + num_periods, :]
        ...
        # losses, mape_list = train(X_all, y_all, args, quantiles)
        # if args.show_plot:

...
class DFRNN(nn.Module):
    ...
    def sample(self, X, num_samples=100, device='cpu'):
        if isinstance(X, type(np.empty(2))):
            X = torch.from_numpy(X).float()
        mu, var = self.forward(X)
        num_ts, num_periods = mu.size()
        z = torch.zeros(num_ts, num_periods).to(device)
...
def train(X, y, args):
    ...
    model = DFRNN(num_features, args.noise_nlayers, 
        ...
        args.global_hidden_size, args.n_factors).to(device)
    ...
    for epoch in tqdm(range(args.num_epoches)):
        ...
        for step in range(int(len(X_train_all)/args.batch_size)):
            ...
            y_pred = model.sample(Xf, device=device)
            loss = util.gaussian_likelihood_loss(ytrain_tensor, mu, sigma)
            mse_l = mse_loss(y_pred, yf)
    ...
    # test
    model = model.cpu()
    pickle.dump(model, open("deepF_{}.pkl".format(time.strftime("%m%d%H%M", time.localtime())), 'wb'))
    mape_list = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seq_len", "-sl", type=int, default=24)  # 预测时序长度
    parser.add_argument("--num_obs_to_train", "-not", type=int, default=24*2)  # 训练输入长度
    ...
    parser.add_argument("--num_days", "-ds", type=int, default=8)
